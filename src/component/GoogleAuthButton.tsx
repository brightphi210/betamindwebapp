...
const GoogleAuthButton = ({
  ...
  const { addToast, setUser } = useGlobalContext();
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        addToast("Failed to get user info. Please try again.", "error");
      } finally {
  ...
  return (
    <button
      ...
      className={`w-full flex items-center justify-center gap-3 py-3.5 px-5 ${rounded} font-medium text-white text-sm transition-all duration-200 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed`}
      style={{
    ...
    >
      {googleLoading ? (
        ...
        <svg width="18" height="18" viewBox="0 0 48 48" fill="none">
          ...
          />
          <path
            d="M43.611 20.083H42V20H24v8h11.303a11.996 11.996 0 01-4.087 5.571l.003-.002 6.19 5.238C36.971 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917z"
            fill="#1976D2"
          />
        </svg>
      )}
      {googleLoading ? loadingText : text}
    </button>
  );
};
